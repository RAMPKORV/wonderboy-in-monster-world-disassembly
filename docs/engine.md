# Wonder Boy in Monster World Ã¢ÂÂ Engine Deep Dive
...
Key ranges (see also ÃÂ§11 RAM map):
...
MainInit:
  ...
  SetupVDP ($4A7A)                ; VDP regs + plane table (see ÃÂ§1)
  clear VRAM
...
for the common ones live in `src/engine/scroll_vdp.asm` ($716/$72C/$74E/$764/$786/$79C).
...
4. `WaitForVBlankScanline ($4C00)` + `FrameWait ($3B8)` Ã¢ÂÂ sync to the raster.
...
interpreter (ÃÂ§7). The *type entry* (geometry) is what the plane set-up uses.
...
### 4.3 "Exposing the plane" Ã¢ÂÂ the full load pipeline

When a scene change is triggered (door, start of game, shop entry), the engine
runs these in order:

**Step 1 Ã¢ÂÂ tile upload (scene tile loader, $22F0 + loc_22FE).**
A scene's tiles come from the **flagged table** (see ÃÂ§4.4). The loader walks a
...
**Step 2 Ã¢ÂÂ tilemap decode (DecodeMap $6D9C).**
...
**Step 3 Ã¢ÂÂ palette load.** `LoadPalettes/DecodePalette` ($135C/$1370) decode the
packed palettes (17 bytes each, see ÃÂ§8) into `$FF8B56` (source) / `$FF8BD6`
(working) and upload to CRAM.

**Step 4 Ã¢ÂÂ render the tilemap to Plane A.** Two writers:
...
- `loc_2082`: bulk renderer Ã¢ÂÂ iterates the visible rows/cols of the map buffer
  and streams them to Plane A, honouring a dirty/skip mask (`$FF8DA0-$FF8DA6`).
- `loc_1758`: full-scene render Ã¢ÂÂ used on scene entry to draw the whole map
  (it computes the scene centre and calls the bulk renderer).

**Step 5 Ã¢ÂÂ home the camera.** `UpdateScrollRegs ($2110)`:
```
...
### 4.4 The flagged table ($41000-$41B40) Ã¢ÂÂ scene building blocks
...
nibble->byte expansion is done with `TileBitSpreadTables` ($6D1C) Ã¢ÂÂ four 16-word
...
4. **Re-exposes the plane** with the new scene: tile upload (ÃÂ§4.3 step 1, using
   the tile-block cache so shared blocks skip), map decode (ÃÂ§4.3 step 2),
   palette, full-scene render (ÃÂ§4.3 step 4 via loc_1758), camera home (ÃÂ§4.3
   step 5).
...
- `DispatchTable1` ($15A0) Ã¢ÂÂ codes < $0C map to the in-scene commands.
- `DispatchTable2` ($15C8) Ã¢ÂÂ codes >= $0C via a second jump table.
...
into `$FF8CA8` so the interpreter resumes where it left off each frame Ã¢ÂÂ this is
...
6. `UpdateScrollRegs` Ã¢ÂÂ write H/V scroll from player + scene origin.
...
This is the same family of engine as the in-scene bytecode (loc_14F4) Ã¢ÂÂ a
...
- Command mailbox: write byte to `$A01C30 + (cmd & $F)` in Z80 RAM
  (SendZ80Command, $364).
- `src/z80/z80_driver.asm` is a full annotated Z80 disassembly (4148 instructions,
...
5. **$02482C 0xFF7A/0xFF7B blocks**: word-offset header + marker blocks
   (palette/animation data) Ã¢ÂÂ encoding still open.
